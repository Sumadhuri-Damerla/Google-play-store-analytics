...
# graph
mice_plot <- aggr(new_play_store, col=c('navyblue','yellow'),
...
# mputed_Data <- mice(new_play_store, m=1, maxit = 50, method = 'pmm', seed = 500)
# summary(mputed_Data)
...
new_play_store$size_mb[selectedRows] <- as.numeric(new_play_store$size_mb[selectedRows])  / 1000
...
# new_play_store$installs <- gsub(',', '', new_play_store$installs)

new_play_store$installs <- gsub('[[:punct:]]', '', new_play_store$installs)

new_play_store$price <- gsub( "as.character($)", '', new_play_store$price, fixed = FALSE)
